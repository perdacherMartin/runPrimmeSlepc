...
PetscErrorCode EPSSetUp_PRIMME(EPS eps)
{
  ...
  else eps->ncv = primme->maxBasisSize = primme->maxBasisSize + eps->nev;
...
// setting print log level 
//  ops->primme.printLevel = 2;
...
PetscErrorCode EPSDestroy_PRIMME(EPS eps)
{
  ...
  //ierr = PetscObjectComposeFunction((PetscObject)eps,"EPSPRIMMESetInitialGuesses_C",NULL);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
#undef __FUNCT__
#define __FUNCT__ "EPSPRIMMESetInitialGuesses_PRIMME"
// matGuesses contains the intitial guesses for the eigenvectors,
// each row (global index) is one vector
PetscErrorCode EPSPRIMMESetInitialGuesses_PRIMME(EPS eps, Mat matGuesses){
	PetscErrorCode ierr;
	EPS_PRIMME     *ops = (EPS_PRIMME*)eps->data;
	primme_params  *primme = &ops->primme;
	PetscInt *idxm, *idxn,i,N=primme->n, guessCount, matGuessesN,j;
	PetscScalar *myguesses;
	
	PetscFunctionBegin;	
	ierr = MatGetSize(matGuesses, &guessCount, &matGuessesN);
	
	guessCount = guessCount > eps->nev ? eps->nev : guessCount ;
	
	if ( guessCount > N ){
		SETERRQ(PetscObjectComm((PetscObject)eps),PETSC_ERR_SUP,"PRIMME: Your initial guesses of eigenvectors have more rows than the matrix.");
	}else if ( guessCount == 0 ){
		SETERRQ(PetscObjectComm((PetscObject)eps),PETSC_ERR_SUP,"PRIMME: Your initial guesses of eigenvectors have zero rows.");
	}else if ( guessCount < 0 ){
		SETERRQ(PetscObjectComm((PetscObject)eps),PETSC_ERR_SUP,"PRIMME: Your initial guesses of eigenvectors are negative.");
	}else if ( matGuessesN != N ){
		SETERRQ(PetscObjectComm((PetscObject)eps),PETSC_ERR_SUP,"PRIMME: Your initial guesses of eigenvectors are not equal to the count of columns of your matrix");
	}
	ierr = PetscMalloc( N * sizeof(PetscInt),&idxn);CHKERRQ(ierr);
	ierr = PetscMalloc( N * sizeof(PetscInt),&idxm);CHKERRQ(ierr);
	ierr = PetscMalloc( guessCount * N * sizeof(PetscScalar),&myguesses);CHKERRQ(ierr);
	
	for ( i = 0 ; i < N ; i++ ){ idxn[i] = i; }
	
	for ( i = 0 ; i < guessCount ; ++i ){
		for ( j = 0 ; j < N ; ++j ){ idxm[j]=i; }

		MatGetValues(matGuesses, 1, idxm, N, idxn, &(myguesses[i*N]));
		VecSetValues(eps->V[i], N, idxn, &myguesses[i*N], INSERT_VALUES);
		VecAssemblyBegin(eps->V[i]);
		VecAssemblyEnd(eps->V[i]);
	}
	
	ierr = PetscFree(idxn);CHKERRQ(ierr);
	ierr = PetscFree(idxm);CHKERRQ(ierr);
	ierr = PetscFree(myguesses);CHKERRQ(ierr);	
	eps->nini = guessCount;
	PetscFunctionReturn(0);
}

#undef __FUNCT__
#define __FUNCT__ "EPSPRIMMESetRandomGuesses_PRIMME"
// matGuesses contains the intitial guesses for the eigenvectors,
// each row (global index) is one vector
PetscErrorCode EPSPRIMMESetRandomGuesses_PRIMME(EPS eps){
	PetscErrorCode ierr;
	EPS_PRIMME     *ops = (EPS_PRIMME*)eps->data;
	primme_params  *primme = &ops->primme;
	PetscInt i;
	PetscRandom rctx;
	PetscScalar *myguesses,*ptr;
	
	PetscFunctionBegin;	
	
	if ( eps->ncv == 0 || eps->nev == 0  ){
		SETERRQ(PetscObjectComm((PetscObject)eps),PETSC_ERR_SUP,"PRIMME: You have to call EPSSetDimensions before EPSPRIMMESetRandomGuesses_PRIMME");
	}
...
//#undef __FUNCT__
//#define __FUNCT__ "EPSPRIMMESetInitialGuesses"
//PetscErrorCode EPSPRIMMESetInitialGuesses(EPS eps, Mat matGuesses)
//{
//  PetscErrorCode ierr;
//
//  PetscFunctionBegin;
//  PetscValidHeaderSpecific(eps,EPS_CLASSID,1);
//  PetscValidLogicalCollectiveMat(eps,matGuesses,2); 
//  ierr = PetscTryMethod(eps,"EPSPRIMMESetInitialGuesses_C",(EPS,Mat),(eps,matGuesses));CHKERRQ(ierr);
//  PetscFunctionReturn(0);
//}
...
PetscErrorCode EPSSetFromOptions_PRIMME(EPS eps)
{
  ...
  KSP             ksp;
/*  
  char            initialGuessesFilename[PETSC_MAX_PATH_LEN]="";
  Mat             matGuesses;
  Viewer          viewer;
*/
  PetscFunctionBegin;
...
/*  
  ierr = PetscOptionsGetString(PETSC_NULL,"-eps_primme_initial_guesses",initialGuessesFilename,PETSC_MAX_PATH_LEN-1,&flg);CHKERRQ(ierr);
  if (flg){
	ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,initialGuessesFilename,FILE_MODE_READ,&viewer);CHKERRQ(ierr);
	ierr = MatCreate(PETSC_COMM_WORLD, &matGuesses);CHKERRQ(ierr);
	ierr = MatSetFromOptions(matGuesses);CHKERRQ(ierr);
	ierr = MatLoad(matGuesses,viewer);CHKERRQ(ierr);
	ierr = PetscViewerDestroy(&viewer);CHKERRQ(ierr);
	
	ierr = EPSPRIMMESetInitialGuesses(eps,matGuesses);CHKERRQ(ierr);
  }
*/
...
PETSC_EXTERN PetscErrorCode EPSCreate_PRIMME(EPS eps)
{
  ...
  ierr = PetscObjectComposeFunction((PetscObject)eps,"EPSPRIMMESetInitialGuesses_C",EPSPRIMMESetInitialGuesses_PRIMME);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
